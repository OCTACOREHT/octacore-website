      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-10">
        <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="space-y-2">
            <p className="text-sm uppercase tracking-[0.2em] text-foreground/60">Octacore Insights</p>
            <h1 className="text-4xl font-semibold text-foreground md:text-5xl">Blog</h1>
        ...
        {posts.length === 0 ? (
          <div className="rounded-2xl border border-foreground/10 bg-foreground/5 px-6 py-10 text-center text-foreground/80 shadow-[0_20px_60px_rgba(0,0,0,0.35)]">
            ...
            {posts.map((post) => {
              ...
              return (
                <Link
                  key={post.id}
                  href={`/blog/${post.slug}`}
                  className="group relative overflow-hidden rounded-2xl border border-foreground/10 bg-gradient-to-br from-white/8 via-white/5 to-transparent p-6 shadow-[0_20px_60px_rgba(0,0,0,0.35)] transition hover:-translate-y-1 hover:border-core-electric/70"
                >
